...
pub async fn produce_handler(
    ...
    let mut f = OpenOptions::new()
        .append(true)
        .open(file_path)
        .expect("Error opening log file for topic");
    ...
    match offset {
        Ok(Some(offset_str)) => {
            let offset_num = offset_str
                .parse::<u32>()
                .expect("Error parsing offset number");
            ...
            let data_string =
                serde_json::to_string(&data_to_store).expect("Error stringify object");
            ...
            match written {
                Ok(_) => {
                    ...
                    state
                        .bucket_directory
                        .producer_bucket
                        .set(&topic, &num.to_string())
                        .expect("Error updating the offset in bucket");
